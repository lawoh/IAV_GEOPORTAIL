...
ol.control.GeoBookmark = function(options) {
  ...
  if (options.target) {
    ...
    this.button = document.createElement('button');
    this.button.setAttribute('type', 'button');
    this.button.setAttribute("title", "Bookmarks");
    this.button.addEventListener('click', function() {
...
ol.Overlay.Tooltip.prototype.formatArea = function(area) {
  if (area > Math.pow(10,-1*this.get('maximumFractionDigits'))) {
    if (area>10000) {
      return (area/10000).toLocaleString(undefined, {maximumFractionDigits: this.get('maximumFractionDigits)')}) + ' Hectare';
    } else {
      return area.toLocaleString(undefined, {maximumFractionDigits: this.get('maximumFractionDigits')}) + ' metre carre';
    }
  } else {
    return '';
  }
